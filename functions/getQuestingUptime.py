...
def getQuestingUptimeByProfession(tablesManager: TablesManager, profession: str):
    ...
    allAccounts = tablesManager.accounts.scan(
        FilterExpression="enabled_quester=:enabled_quester AND profession=:profession AND disabled <> :disabled",
    ...
    questingAccounts = tablesManager.accounts.scan(
        FilterExpression="questing=:questing AND profession=:profession AND disabled <> :disabled",
        ExpressionAttributeValues={
            ":questing": True,
            ":disabled": True,
            ":profession": profession
